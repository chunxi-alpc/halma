...
def draw():
    ...
    if ai_mode == 1 and role == ai.other_man and input_flag == 0:
        if ans != []:
...
class AI:
    ...
    def alphaBeta_search(self, depth, alpha, beta, player):
        ...
        for node in nodes:
            chess, space = node
            chess_x, chess_y = chess#棋子
            space_x, space_y = space#空格
            ...
            if value > alpha:  # val大于了下界alpha，修改alpha，这是一个PV节点
                if depth == gameside:  # 当出现了优于目前的alpha值则记录最佳走法
                    self.best_move = node
                alpha = value
        return alpha
    ...
    while True:
        for e in pygame.event.get():
            ...
            if e.type == pygame.QUIT:
                ...
                key_Processing(e)
            elif ai_mode==1 and net_flag==0:
                # 模式切换，网络切换过来应该先退出，然后重新初始化
                expression=''
                ai.best_move = -1
                print(ai.alphaBeta_search(gameside, -100000, 100000, role))
                ...
                flag[space_x][space_y] = copy.deepcopy(flag[chess_x][chess_y])
                if flag[chess_x][chess_y] > 9:
                    send_num =flag[chess_x][chess_y]-10
                else:
                    send_num = flag[chess_x][chess_y]
                flag[chess_x][chess_y] = -1
